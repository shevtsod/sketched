...
describe('ResetCommand', () => {
  ...
  it('should not run without confirm', async () => {
    ...
    expect(dbManagementService.reset).not.toHaveBeenCalled();
  });

  it('should run', async () => {
    await command.run([], { confirm: true });
    expect(dbManagementService.reset).toHaveBeenCalled();
  });

  it('should throw when run fails', async () => {
    const error = new Error('Test Error');
    dbManagementService.reset.mockImplementationOnce(() => {
      throw error;
    });
